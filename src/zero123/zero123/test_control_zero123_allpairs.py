        ...
        if os.path.basename(subdir) == "thumbnails":
            for image1 in images:
                for image2 in images:
                    ...
                    angle_difference = get_angle_difference(image1, image2)
                    
                    if use_depth:
                        depth_map = Image.fromarray(get_depth(os.path.join(subdir, image2), midas, device))
                    else:
                        depth_map = None
                    ...
                    results = main_run(
                        ...
                        raw_im=Image.open(os.path.join(subdir, image1)),
                        target_im=Image.open(os.path.join(subdir, image2)),
                        depth_map=depth_map,
                        preprocess=True,
                    ...
                    # Construct the filename to indicate the chosen images and angle difference
                    new_filename = f"{image1.split('.')[0]}_{image2.split('.')[0]}_angle{angle_difference}{suffix}.jpg"
