...
class StatelessEulerAncestralDiscreteScheduler(SchedulerMixin, ConfigMixin):
    """
    ...
    def set_timesteps(
        ...
        if timesteps is not None:
            ...
            self.timesteps = torch.tensor(timesteps, dtype=torch.float32).to(device)
    ...
    def index_for_timestep(self, timestep, schedule_timesteps=None):
        if schedule_timesteps is None:
            ...
            schedule_timesteps = self.original_timesteps
    ...
    def step(
        ...
        if self.config.prediction_type == "epsilon":
            ...
            pred_original_sample = model_output * (-sigma_from / (sigma_from**2 + 1) ** 0.5) + (sample / (sigma_from**2 + 1))
