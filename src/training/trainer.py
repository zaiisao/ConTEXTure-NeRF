...
class TEXTure:
    ...
    def paint_zero123plus(self):
        ...
        self.outputs = self.mesh_model.render(theta=self.thetas, phi=self.phis, radius=self.radii, background=background)
        ...
        depth_rgba = torch.cat((depth, depth, depth, masks), dim=1)

        cropped_depth_rgba = utils.crop_mask_to_bounding_box( depth_rgba, bounding_boxes)
                
        cropped_rgb_render =  utils.crop_mask_to_bounding_box(self.outputs['image'], bounding_boxes) 
        ...
        max_cropped_image_height = max([box[2] - box[0] for box in bounding_boxes])
        max_cropped_image_width = max([box[3] - box[1] for box in bounding_boxes])
        #MJ:  bounding_boxes[i, :] = torch.tensor([min_h, min_w, max_h, max_w])   

        cropped_depths_rgba = F.interpolate(
            cropped_depth_rgba,
        ...
        def on_step_end(pipeline, i, t, callback_kwargs):
            ...
            for v in range( len(self.thetas) ): #MJ: v=0,1,...,6: 
                ...
                diff = (cropped_rgb_renders[v-1].detach() - torch.tensor(self.mesh_model.default_color).view(1, 3, 1, 1).to(
