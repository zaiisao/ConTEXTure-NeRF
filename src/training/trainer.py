    ...
    def paint_zero123plus(self):
        ...
        def on_step_end(pipeline, i, t, callback_kwargs):
            ...
            for viewpoint_index, data in enumerate(self.dataloaders['train']):
                ...
                preprocessed_rgb_render_small = pipeline.image_processor.preprocess(rgb_render_small)

                gt_latents = scale_latents(
                    pipeline.vae.encode(
                        scale_image(preprocessed_rgb_render_small.half()),
                        return_dict=False
